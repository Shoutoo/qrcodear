...
2. **PENTING — route yang benar**: fitur cetak kartu QR yang sudah ada
   memakai pola route `/print/:id` (BUKAN `/print-preset/:id`). Untuk
   preset ekosistem, route yang benar adalah:
   - `/print/preset-darat`
   - `/print/preset-hutan`
   - `/print/preset-laut`
   - `/print/preset-sawah`

   Tombol "Cetak Barcode" di tiap card harus link ke route `/print/`
   ini (bukan bikin prefix baru `/print-preset/`). Cek dulu handler
   route `/print/:id` yang sudah ada — kemungkinan besar dia sudah
   bisa menangani ID preset ini juga (tinggal pastikan `id` yang
   dikirim match dengan key preset di
   `server/data/ecosystem-presets.json`, mis. `preset-darat`), atau
   kalau belum, tambahkan handling untuk ID berformat `preset-*` di
   route yang sama.
3. Cari fitur kartu cetak QR yang sudah ada (yang sudah dipakai untuk
...
4. Cek apakah backend punya endpoint untuk generate QR image dari
...
5. Pastikan URL yang di-encode ke QR itu URL PUBLIC yang bisa diakses
